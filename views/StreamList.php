<?php
// FILE: views/stream/StreamList.php
?>
...
<div class="container-stream">
    ...
    <h2 class="page-title">Daftar Stream</h2>

    <a href="index.php?entity=stream&action=add" class="btn btn-primary">
        Tambah Stream
    </a>
            ...
            <table class="data-table">
                <thead>
                    <tr class="table-header-row">
                        ...
                        <th class="table-header-cell">Judul Stream</th>
                ...
                <?php if (!empty($streamList)): ?>
                    <?php foreach ($streamList as $stream): ?>
                        <tr class="table-data-row">
                            ...
                            <td class="table-data-cell">
                                <?= htmlspecialchars($stream['vtuber_name']); ?>
                            </td>

                            <td class="table-data-cell">
                                <?= date("Y-m-d", strtotime($stream['stream_date'])); ?>
                            </td>

                            <td class="table-data-cell action-links">
                                <a href="index.php?entity=stream&action=edit&id=<?= $stream['id']; ?>" 
                                   class="action-edit">
                                    Edit
                                </a>
                                |
                                <a href="index.php?entity=stream&action=delete&id=<?= $stream['id']; ?>"
                                   onclick="return confirm('Hapus stream \"<?= htmlspecialchars($stream['title']); ?>\"?');"
                                   class="action-delete">
                                    Hapus
                                </a>
                            </td>
